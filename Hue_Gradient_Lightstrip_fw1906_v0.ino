...
void convertCt(uint8_t light) // convert ct (color temperature) value from HUE API to RGB
{
  ...
  int optimal_bri = int(3 + bri / 1.01);
...
RgbwwColor convFloat(float color[5]) { // return RgbColor from float
  ...
  if (inTransition) {
    if (lightsCount == 1) {
      ...
      for (uint8_t pixel = 0; pixel < pixelCount; pixel++) {
        ...
        strip->SetPixelColor(pixel, RgbwwColor::LinearBlend(convFloat(lights[light].currentColors), convFloat(lights[light + 1].currentColors), (float)(pixel - (light * lightLedsCount)) / (float)lightLedsCount));
      }
    }
    strip->Show();
...
void setup() {
  ...
  server.on("/detect", []() { // HTTP GET request used to discover the light type
    ...
    root["modelid"] = "LCX004";
    root["type"] = "fw1906_gradient_lightstrip";
  ...
  if (packetSize) { // if nr of bytes is more than zero
    ...
    for (uint8_t light = 0; light < lightsCount; light++) {
      ...
      if (light == 0) {
        if (lightsCount == 2) {
          for (uint8_t pixel = 0; pixel < pixelCount; pixel++) {
            strip->SetPixelColor(pixel, RgbwwColor::LinearBlend(convFloat(lights[0].currentColors), convFloat(lights[1].currentColors),  (float)(pixel) / (float)pixelCount));
          }
        } else {
          for (uint8_t pixel = 0; pixel < lightLedsCount; pixel++) {
            strip->SetPixelColor(pixel, RgbwwColor::LinearBlend(convFloat(lights[0].currentColors), convFloat(lights[1].currentColors),  (float)(pixel) / (float)lightLedsCount));
          }
        }
      } else if (light != lightsCount - 1) {
        for (uint8_t pixel = 0; pixel < lightLedsCount ; pixel++) {
          strip->SetPixelColor(pixel + lightLedsCount * light, RgbwwColor::LinearBlend(convFloat(lights[light].currentColors), convFloat(lights[light + 1].currentColors), (float)(pixel) / (float)lightLedsCount));
        }
      }
